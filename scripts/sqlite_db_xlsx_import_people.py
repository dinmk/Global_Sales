...
def insert_values_to_table(table_name, xl_file):
    ...
    if conn is not None:
        ...
        c.execute('CREATE TABLE IF NOT EXISTS ' + table_name +
                  '(tbl_stg_people_person      VARCHAR,'
                  'tbl_stg_people_region    VARCHAR)'
                  )

        df = pd.read_excel(xl_file,sheet_name='People')
...
if __name__ == '__main__':
    insert_values_to_table('tbl_stg_people', XL_FILE_PATH)
